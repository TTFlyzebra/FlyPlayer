 ...
 * 去掉高斯模糊背景
 */
public class MusicActivity_AP8 extends MusicActivity_AP1 {
    protected ImageView ac_music_bkimg;
    ...
    protected void upCurrenPlayImage(final Bitmap bitmap) {
        ...
        executor.execute(new Runnable() {
            @Override
            public void run() {
                final Bitmap blurBitmap = bitmap == null ? null : BlurUtil.blur(MusicActivity_AP8.this, bitmap);
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        ac_music_bkimg.setImageBitmap(null);
                    }
                });
            }
        });
